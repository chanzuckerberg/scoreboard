...
passport.use(
  new GitHubStrategy(
    ...
    function (accessToken, refreshToken, profile, cb) {
      ...
      let email = "";
      if ("emails" in profile && profile.emails.length)
...
app.get(
  "/auth/github",
  passport.authenticate("github", { scope: ["user:read"] })
);
...
app.post("/api/submitresults", [
  ...
  check("repo")
    ...
    .withMessage(
      "Github Repo is required and must be a valid link to a github repository"
    ),
  check("publications")
    .optional()
    .custom((value) => {
