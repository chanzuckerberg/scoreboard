...
function getSubmissions(req, res, next) {
	...
	db
		.any(
			"select u.name as user_name, s.*, r.results_path, r.score_data from submissions s join users u on u.id = s.user_id join results r on s.id = r.submission_id where s.challenge_id = $1 and r.is_current = True",
			challengeID
		)
		.then(function(data) {
			res.status(200).json({
