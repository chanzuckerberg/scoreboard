...
function getOneChallenge(req, res, next) {
  ...
  db.any("select c.* " + "from challenges c " + "where c.id = $1 ", challengeID)
    .then(function (data) {
...
function approveAlgorithm(req, res, next) {
  ...
  if (!req.user.is_admin) {
    return res
      .status(401)
      .json("User does not have permission to approve or reject algorithm");
  }
  ...
  if (approved) {
    db.one(
      "update submissions set is_accepted=true where id=$1 RETURNING id",
      submissionId
    )
      .then((data) => {
        db.one(
          "select u.* from users u inner join submissions s on s.user_id=u.id WHERE s.id=$1",
          submissionId
        )
          .then((submitterEmail) => {
    ...
    db.one("delete from submissions where id=$1 RETURNING id", submissionId)
      .then((data) => {
        res.sendStatus(204);
      })
      .catch((err) => {
...
function submitResults(req, res, next) {
  ...
  // Score with docker
  db.one(
    "select docker_container from challenges where id = $1",
      ...
      (err, stdout, stderr) => {
        if (err) {
          ...
          if (results["error"] !== "") {
            ...
            if ("additionalData" in results)
              score.additionalData = results.additionalData;
